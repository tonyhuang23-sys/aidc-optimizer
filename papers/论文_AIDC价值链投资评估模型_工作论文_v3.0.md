...
### 1.3 贡献声明（依评估意见二弱化首创性表述）

本文不声称发明CVaR、copula或增量回报指标；其贡献在于**首次将Role、Asset Stack、Contract、Project Finance、相关性风险模拟与Sponsor级资本配置系统整合进统一的AIDC价值链投资框架**，并以完整因子实验与三级验证支持其结论的可复现性。
...
**（4）融资审读指标**。Equity NPV@Ke、MIRR、TVPI/MOIC、Peak Equity、现金流符号翻转计数；Project NPV<0而Equity IRR>0时标记**Leverage-distorted IRR**；Min DSCR<covenant标记**Not financeable under covenant**。并定义**利益相关方总NPV = Equity NPV + Debt NPV**（§6.4）。
...
†开发退出报MOIC 2.00×、年化XIRR 36%、27个月。电力层P1的MC未单独跑满（损失率由P3的0/2000与利差结构推断为低），正式版补齐并附CI。
...
**两项结论修正（相对v2.0）**：(1) v2.0摘要"合约>代际>资产栈"基于不完整格子；完整因子分解的正确排序为**GPU代际＞合约结构≫资产栈**（P0-4勘误）；(2) 新发现**合约×代际负交互**——长约与二手卡的价值来源部分重叠（都是抗衰减），不可叠加受益。**八格全部NPV为负**：即使最优格（棕地×长约×二手，-5.3% IRR）仍未跨越盈亏平衡，租金模式自持GPU在全部维度组合中不成立——其转正需token模式收入（L5a口径，§5.5）或模型层角色（L5b）。
...
即股东每多投1美元峰值股本于GPU层，毁灭约5.8美元确定性等价价值。（注：若用表内四舍五入值手工复算会得到-5.46；差异源于表格展示精度，正文一律采用底层精确值。）
...
短缺情景完整指标组（修v2.0表6数据，结论不变）：NPV -$1,412M、Project IRR -2.5%、Equity IRR 58.6%[Leverage-distorted]、Equity NPV@Ke +$1,492M、MIRR 25.9%、TVPI 6.38×、Min DSCR 1.48；Vendor Backstop情景Min DSCR 0.90×→**Not financeable under covenant**。
...
### 6.2 融资再分配≠价值创造（依评估意见二§11新增为正式框架）

短缺情景呈现Project NPV(-$1.4B)与Equity NPV@Ke(+$1.5B)并存。按守恒关系：

$$\underbrace{Project\ NPV}_{-1.4B} = \underbrace{Equity\ NPV}_{+1.5B} + \underbrace{Debt\ NPV}_{\approx -2.9B}$$

即**股权的正价值来自融资合同对债权人价值的再分配，而非项目创造价值**。任何商业模式必须同时检查三个NPV；仅报告Equity IRR的模型（市场上大量存在）会系统性误导。这对GPU/private-credit/vendor-backstop模式尤为重要——真实的CoreWeave式定价（9.25%+GPU抵押+厂商容量回购）正是市场对Debt NPV为负风险的补偿[4][13]。
...
**RQ3**：完整2×2×2因子实验给出主效应排序**GPU代际(+$1.45B)＞合约结构(+$0.91B)≫资产栈(+$0.25B)**，且合约×代际次可加（-$0.45B）；八格全负，三维全部最优仍不翻转租金模式——GPU层正经济性属于具备模型层角色或token变现能力的参与者。
...
后续工作（v4.0）：交易级样本外验证集（≥5笔/层）、L5a token转换引擎、Debt NPV/Stakeholder NPV的系统输出、参数相关性事后校准、lease-tail/balloon压力测试、CME租金期货对冲模块、MACRS/CCA与DSRA银行级实现。
...
（同v2.0附录A；新增P1情景参数：新建CCGT $3,500/kW取Greenlight CA$4.6B/932MW×0.714折算[8]，置信0.85。）
...
**AI辅助披露**：模型实现、数据分析与初稿写作由AI辅助完成，全部数值结果可由随附代码复现；v3.0依外部评估意见二修订，P0项（公式、算术、日期、排序）已逐项核对。
