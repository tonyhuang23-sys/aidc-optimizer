...
**（3）相关性蒙特卡洛**。GPU四参数（租金率σ=0.25、残值σ=0.35、采购价σ=0.15、利用率σ=0.10）经高斯copula联合采样，相关矩阵ρ(租金,残值)=0.6、其余0.25。各表注明实际N；损失概率一律附**Wilson 95%置信区间**（如0/1500观察损失→95%CI 0–0.26%；800/800→95%CI 99.52–100%），不再表述为0%或100%。
...
†开发退出报MOIC 2.00×、年化XIRR 36%、27个月。电力层P1的MC未单独跑满（损失率由P3的0/2000与利差结构推断为低），正式版补齐并附CI。‡v3.3起"峰值股本"列统一按**Peak Equity**（累计股本流出峰值，即风险资本占用的真实峰值）报告，跨层口径一致（v3.2中DC层按Sponsor Equity报告导致的跨层不一致已修正，如L3由287改为317）；IRASR与§5.4均采用同一Peak Equity口径，读者可用表值复算。

辅助增量链（ΔNPV/ΔCAPEX，基于表4值）：Dev→Power +0.28；Power(P1)→Shell −0.17（**边际/负值**——市场一致电力基准下，壳体层几乎不创造增量价值）；Shell→Critical IT +2.04；Critical IT→Hosting +0.08；Hosting→GPU −0.86。与v2.0相比的重要变化：**电力层回到基础设施回报量级（14.1%）后，"Power→Shell无增量"的判断更加突出**，价值创造的重心明确落在"绿电→AI-ready"的转换环节。
...
*GPU租金率（$/IT-kW-hr）：新卡现货$3.0/长约$3.2，二手现货$1.9/长约$2.0；二手GPU $12k/IT-kW、残值0.30；长约：衰减-8%、利用率95%；资产栈：绿地 power.capex $1,150/kW+DC ready $8,000/kW-IT，棕地 power $150/kW+ready $5,500/kW-IT；其余同基准。*
...
**L3>L4的排序在全部λ取值下不变**——甜蜜点结论不是λ=0.5人为选取的结果（λ从0到1仅使L3的CE-NPV从+706收窄至+304，始终为正；L4在全部λ下为负）。

**IRASR（基于未四舍五入底层结果）**：

$$IRASR_{L3\to L4(短缺)} = \frac{-2,528.5}{+438.8} = \mathbf{-5.76}$$

分母的两个底层值（**Peak Equity口径**）：L3峰值股本$316.5M、L4短缺情景峰值股本$755.3M，Δ=+438.8M（未四舍五入值$316.511M与$755.294M）。表4"峰值股本"列自v3.3起统一采用同一Peak Equity口径，读者用表值复算（755−317=438）与-5.76一致（v3.2中DC层按Sponsor Equity报告导致的-5.46口径偏差已消除）。即股东每多投1美元峰值股本于GPU层，毁灭约5.8美元确定性等价价值。（注：正文一律采用底层精确值。）
...
（1）筛选级精度（重要性已量化于§6.4）；（2）参数2026-08快照；（3）USD单一币种；（4）相关矩阵为经验设定（ρ(租金,残值)=0.6、其余0.25）；（5）Level C为合理性检验而非样本外预测验证（v4.0目标：≥5笔交易的Prediction-vs-Actual，报告MAE/MAPE/偏差）；（6）L5a为收入等效折算而非token单位经济模型；（7）L5b/Captive不建模（其存在意味着GPU层负结论不可外推至模型公司与hyperscaler自建决策）；（8）电力层P1的MC样本量未与其他层对齐。

---

## 7 结论

**RQ1**：市场一致口径下（电力层P1），七模式排序为Critical IT租赁与GPU托管（CE-NPV +$505M、观察损失0/1500）＞Power-only基础设施回报（14.1% EqIRR）≈Shell（边际）；租金模式自持GPU与L5a在基准下深度为负。

**RQ2**：IRASR在L3→GPU层为**-5.76**（短缺情景、未四舍五入值）；辅助增量链显示Power(P1)→Shell为负（-0.17）——价值创造重心在"电力→AI-ready"转换环节，基准停留点L3–L3.5。

**RQ3**：完整2×2×2因子实验给出主效应排序**GPU代际(+$1.45B)＞合约结构(+$0.91B)≫资产栈(+$0.23B)**（经济情景包口径），且合约×代际存在经济上较大的负交互（-$0.45B，次可加）；八格在本文参数范围内全负，三维全部最优仍不翻转租金模式——GPU层正经济性属于具备模型层角色或token变现能力的参与者。
...
（同v2.0附录A；新增P1情景参数：新建CCGT $3,500/kW取Greenlight CA$4.6B/932MW×0.714折算[8]，置信0.85。注明$3,500/kW为**overnight口径**（EPC+业主费用，不含IDC与融资费用）；融资层单独计IDC，P1下IDC≈$7M（CAPEX的1.9%）。若将Greenlight口径解读为含融资成本的全包数字，overnight等价约$3,430/kW，P1 EqIRR约+0.2–0.3pp、NPV约+$5M，结论不变。全部表格可由公开仓库一键复现：`python reproduce.py run` 生成 expected_outputs/alberta.json（黄金结果），`python reproduce.py check` 校验与论文一致。）
...
python reproduce.py run            # 生成 expected_outputs/alberta.json（论文全部表格）
python reproduce.py check          # 校验与论文v3.3一致（exit 0 = 通过）
python tests/test_validation.py    # Greenlight/TeraWulf校准测试
# 单场景/定制：python -m aidc_optimizer --config configs/alberta.yaml --case critical_it --set dc.ready_rent_kw_mo=180
# 说明：2×2×2全因子、P1/P2/P3、短缺情景与λ敏感性全部由 reproduce.py 内置并输出到黄金JSON；
#       相关MC（高斯copula，ρ(rate,residual)=0.6、其余0.25，seed=42）实现于 reproduce.py。
```

---

**模型与数据可得性**：开源仓库 `aidc-optimizer`（GitHub；Python包、脱敏Alberta/Texas参数包、`reproduce.py` 一键复现+黄金结果校验、校准测试）。复现：`pip install -r requirements.txt && python reproduce.py check`。

**利益声明**：作者所属机构从事北美BTM电力与数据中心开发业务，模型参数含机构自有实盘造价数据；P3套利情景与机构业务方向相关，已在§5.1显式分离于市场一致基准P1。

**AI辅助披露**：模型实现、数据分析与初稿写作由AI辅助完成，全部数值结果可由随附代码复现；v3.0依外部评估意见二修订，P0项（公式、算术、日期、排序）已逐项核对；v3.1依评估意见三修订，聚焦口径与归属（情景包因子界定、CE-NPV与WACC职责分工、融资价值归属贴现率口径、L5a结论边界）；v3.2依评估意见四修订，聚焦数值透明与表述限定（IRASR分母底层值披露、Power CAPEX/IDC口径拆分、"显著"改"经济上较大"、八格结论限定、删除"首次"）；v3.3依开源复现对齐修订：全部表格由 `reproduce.py` 从固定代码+config+seed 重跑生成并经 `reproduce.py check` 校验一致。
